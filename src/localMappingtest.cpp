...
LocalMap::LocalMap()
{
  ...
  // 历史帧参数
  nh.param<double>("HISTORY_SIZE/MAX_HISTORY_SIZE", MAX_HISTORY_SIZE, 50);
...
void LocalMap::process()
{
  // 保持一个20帧的队列用于建图的效果
  // 全局定义固定长度队列（容量20）
  std::deque<pcl::PointCloud<pcl::PointXYZI>::Ptr> cloud_history;
  while(ros::ok())
  {
    ...
    // 将当前处理后的点云加入队列
    pcl::PointCloud<pcl::PointXYZI>::Ptr current_cloud(new pcl::PointCloud<pcl::PointXYZI>(*localMap_filtered));
    cloud_history.push_back(current_cloud);

    // 若超过容量，移除最旧的点云
    if (cloud_history.size() > MAX_HISTORY_SIZE) {
        cloud_history.pop_front();
    }

    // 合并历史点云用于后续处理（可选）
    pcl::PointCloud<pcl::PointXYZI>::Ptr merged_cloud(new pcl::PointCloud<pcl::PointXYZI>);
    for (const auto& cloud : cloud_history) {
        *merged_cloud += *cloud;
    }
    local_pcl = merged_cloud; // 更新为合并后的点云
    // icp初值
